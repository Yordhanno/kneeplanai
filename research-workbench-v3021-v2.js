...
const APP_VERSION = 'KneePlanAI Web Research 1.2.0';
const DEFAULT_CALIBRATION_DIAMETER_MM = 25;
const MIN_ZOOM = 65;
const MAX_ZOOM = 3500;
...
const NUDGE_CIRCLES = new Set(['femur_proximal', 'femur_f10', 'tibia_t4', 'tibia_t10']);
...
const COPY = {
  es: {
    ...
    nudgeNeed: 'Seleccione Fémur proximal, F10, T4 o T10.',
    ...
    calibrationStart: 'Calibración manual ({diameter} mm): marque un borde de la esfera.',
    calibrationSecond: 'Calibración manual ({diameter} mm): marque el borde opuesto de la esfera.',
    calibrationInvalid: 'La escala marcada es demasiado pequeña. Repita la calibración.',
    calibrationReady: 'Escala manual: {diameter} mm = {px} px · {spacing} mm/px.',
    calibrationNone: 'Sin calibrar · esfera manual de {diameter} mm por defecto.',
    calibrationDicom: 'Escala DICOM: Pixel Spacing · {spacing} mm/px. La calibración manual es opcional.',
  ...
  en: {
    ...
    nudgeNeed: 'Select proximal femur, F10, T4 or T10.',
    ...
    calibrationStart: 'Manual calibration ({diameter} mm): mark one edge of the sphere.',
    calibrationSecond: 'Manual calibration ({diameter} mm): mark the opposite edge of the sphere.',
    calibrationInvalid: 'The marked scale is too small. Repeat calibration.',
    calibrationReady: 'Manual scale: {diameter} mm = {px} px · {spacing} mm/px.',
    calibrationNone: 'Uncalibrated · manual sphere defaults to {diameter} mm.',
    calibrationDicom: 'DICOM scale: Pixel Spacing · {spacing} mm/px. Manual calibration is optional.',
...
const calibrationDiameterInput = document.getElementById('calibration-diameter');

function calibrationDiameter() {
  const value = Number(calibrationDiameterInput?.value);
  return Number.isFinite(value) && value > 0 ? value : DEFAULT_CALIBRATION_DIAMETER_MM;
}

function fmtDiameter(value) {
...
function renderCalibration() {
  ...
  label.textContent = `${fmtDiameter(state.calibration.diameter_mm || DEFAULT_CALIBRATION_DIAMETER_MM)} mm`;
  referenceLayer.append(label);
}
...
function selectCalibrationTool() {
  ...
  const diameter = fmtDiameter(calibrationDiameter());
  guidance.textContent = activate ? COPY[state.language].calibrationStart.replace('{diameter}', diameter) : COPY[state.language].ready;
  updateToolButtons(); updateRadiusControl();
}
...
function completeCalibration(p1, p2) {
  ...
  const diameter = calibrationDiameter();
  const spacing = diameter / px;
  state.calibration = { p1, p2, px, spacing, diameter_mm: diameter };
  ...
  state.technical.calibration_source = 'manual_fiducial';
  state.technical.calibration_fiducial_mm = diameter;
  state.calibrationStart = null; state.activeTool = null;
...
function placeActive(point) {
  ...
  if (key === 'calibration_25') {
    if (!state.calibrationStart) {
      ...
      guidance.textContent = COPY[state.language].calibrationSecond.replace('{diameter}', fmtDiameter(calibrationDiameter()));
      return;
    }
...
function updateCalibrationStatus() {
  ...
  if (state.calibration) {
    calibrationStatus.textContent = COPY[state.language].calibrationReady
      .replace('{diameter}', fmtDiameter(state.calibration.diameter_mm || calibrationDiameter()))
  ...
  calibrationStatus.textContent = COPY[state.language].calibrationNone.replace('{diameter}', fmtDiameter(calibrationDiameter()));
}
...
function updateRadiusControl() {
  ...
  radiusSlider.value = String(Math.round(item.radius * 10) / 10);
  document.getElementById('radius-value').textContent = `${item.radius.toFixed(1)} px`;
}
...
radiusSlider.addEventListener('input', () => {
  ...
  item.radius = Number(radiusSlider.value); document.getElementById('radius-value').textContent = `${item.radius.toFixed(1)} px`; renderGeometry();
});
...
calibrationDiameterInput?.addEventListener('change', () => {
  if (!(Number(calibrationDiameterInput.value) > 0)) calibrationDiameterInput.value = String(DEFAULT_CALIBRATION_DIAMETER_MM);
  updateCalibrationStatus();
});
...
function nudgeSelectedCircle(direction, coarse = false) {
  const key = state.selectedKey;
  const item = key ? state.geometry[key] : null;
  if (!item || item.type !== 'circle' || !NUDGE_CIRCLES.has(key)) {
    showMessage(COPY[state.language].nudgeNeed, 'error'); return false;
  }
  const screenStep = coarse ? 8 : 2;
  const imageStep = screenStep / Math.max(state.scale, 1e-9);
  item.center[0] = Math.max(0, Math.min(state.width, item.center[0] + (direction > 0 ? imageStep : -imageStep)));
  guidance.textContent = `${LABELS[key]}: ${state.language === 'es' ? 'ajuste horizontal' : 'horizontal adjustment'}.`;
  renderGeometry(); return true;
}

window.addEventListener('kpai:nudge-circle', (event) => {
  nudgeSelectedCircle(Number(event.detail?.direction) || 0, Boolean(event.detail?.coarse));
});
...
window.addEventListener('keydown', (event) => {
  ...
  if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
    const key = state.selectedKey;
    if (key && NUDGE_CIRCLES.has(key)) {
      event.preventDefault(); nudgeSelectedCircle(event.key === 'ArrowRight' ? 1 : -1, event.shiftKey); return;
    }
  }
...
function drawAnnotatedCanvas(maxDimension = 2400) {
  ...
  const lineWidth = Math.max(1.2, Math.min(state.width, state.height) / 420);
  ...
  for (const [, item] of Object.entries(g)) {
    ctx.strokeStyle = '#2ad5b6'; ctx.lineWidth = lineWidth;
    if (item.type === 'circle') { ctx.beginPath(); ctx.arc(item.center[0], item.center[1], item.radius, 0, Math.PI * 2); ctx.stroke(); }
    else if (item.type === 'point') { ctx.beginPath(); ctx.arc(item.position[0], item.position[1], Math.max(2, handleRadius() * .45), 0, Math.PI * 2); ctx.fillStyle = '#f4fbfc'; ctx.fill(); ctx.stroke(); }
    else { ctx.beginPath(); ctx.moveTo(...item.point_1); ctx.lineTo(...item.point_2); ctx.stroke(); }
  }
  if (state.calibration) {
    const mid = midpoint(state.calibration.p1, state.calibration.p2); ctx.font = '700 13px Inter, Arial, sans-serif'; ctx.fillStyle = '#f5c451';
    ctx.fillText(`${fmtDiameter(state.calibration.diameter_mm || DEFAULT_CALIBRATION_DIAMETER_MM)} mm`, mid[0] + 7, mid[1] - 7);
  }
  ctx.restore();
